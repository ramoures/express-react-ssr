...
const Category = ({ dataFromServer }) => {
    ...
    const addToCart = (_v) => {
        ...
        localStorage.setItem('erSSR-shop-cart', JSON.stringify({ items: [...cart, { id: _v?.id, title: _v?.title, price: _v?.price, image: _v?.image?.[0], category: Lowercase(_v?.category?.name) }], prices: [...prices, _v?.price] }));
        if (typeof window !== 'undefined') {
            ...
            {!loading && !error && checkData(data) &&
                <div className="flex flex-col justify-start items-start py-3">
                    <div className="flex w-full justify-between items-center">
                        <Link to={`/category/${Lowercase(name)}`} className={`text-xl w-auto hover:text-neutral-600 ${bgColor} p-4 rounded-t-lg`}>{Capitalize(name)}</Link>
