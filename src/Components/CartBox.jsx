        ...
        <div id="toggleCart" className="flex select-none p-2 items-center gap-1 bg-white relative rounded-lg" onMouseOver={showCartBox} onMouseLeave={hideCartBox}>
            <div className="hidden lg:block">
                <Link to={`/cart`} className="flex gap-1 items-center">
                    ...
                    <div className="text-xs bg-red-600 rounded-full text-white py-0 px-1">{cart?.length > 0 && cart?.length}</div>
                </Link>
            </div>
                        ...
                        {cart.length && cart.map((_v, _i) => {
                            ...
                            return (
                                <div key={_i} className="flex items-center justify-between gap-2 w-full select-none ">
                                    <Link onClick={hideCartBox} to={`/category/${encodeURI(encode(_v?.category))}/products/${_v?.id}`} className="min-w-14 max-w-14 min-h-14 max-h-14 h-auto flex flex-col justify-center items-center border-2 ">
                                    ...
                                    <div className="flex flex-col gap-1 flex-1">
                                        <Link onClick={hideCartBox} to={`/category/${encodeURI(encode(_v?.category))}/products/${_v?.id}`}>{_v?.title}</Link>
